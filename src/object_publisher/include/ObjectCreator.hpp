...
bool try_get_viewed_objects_relative_position(std::vector<Object> &objects, const double &usv_x, const double &usv_y, const double &usv_heading){
        ...
        double half_field_of_view_rad = field_of_view_*0.5*M_PI/180.0;
        ...
        for(const auto &object : objects_){
            ...
            if(distance <= max_radius_ && distance > min_radius_ && angle_diff <= half_field_of_view_rad && std::cos(angle_diff)*distance > 0.0){
